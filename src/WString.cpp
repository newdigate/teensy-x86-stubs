...
{
    unsigned int length = strlen((const char *)pgmstr);
    if (!reserve(length)) {
        if (buffer) {
            free(buffer);
            buffer = NULL;
    ...
    len = length;
    strcpy(buffer, (const char *)pgmstr);
    return *this;
}
...
String & String::append(const __FlashStringHelper *pgmstr)
{
    ...
    unsigned int newlen = len + length;
    if (length == 0 || !reserve(newlen)) return *this;
    strcpy(buffer + len, (const char *)pgmstr);
    len = newlen;
    return *this;
}
...
{
    return strcmp(buffer, (const char *)pgmstr) == 0;
}
...
void String::utoa(unsigned int num, char *buf, int i) {
    sprintf(buf, "%d", num);
}

void String::itoa(int num, char *buf, int i) {
    sprintf(buf, "%d", num);
}
